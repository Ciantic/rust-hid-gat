...
pub fn s1(
    ...
    e(
        k,
        &[
            r1[8], r1[9], r1[10], r1[11], r1[12], r1[13], r1[14], r1[15], r2[8], r2[9], r2[10],
...
pub fn c1(
    ...
    let p2 = [
        0, 0, 0, 0, ia[0], ia[1], ia[2], ia[3], ia[4], ia[5], ra[0], ra[1], ra[2], ra[3], ra[4],
        ra[5],
    ];
...
pub fn c1_rev(
    ...
    let mut r_ = r.clone();
    r_.reverse();
    let mut pres_ = pres.clone();
    pres_.reverse();
    let mut preq_ = preq.clone();
    preq_.reverse();
    let mut ia_ = ia.clone();
    ia_.reverse();
    let mut ra_ = ra.clone();
    ra_.reverse();
    let mut res = c1(&k_, &r_, &pres_, &preq_, iat, &ia_, rat, &ra_);
    res.reverse();
    res
}
...
mod tests {
    ...
    fn test_s1() {
        ...
        let k: [u8; 16] = [0x00; 16];
        let r1: [u8; 16] = [
            0x00, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
            0x77, 0x88,
        ];
        let r2: [u8; 16] = [
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee,
            0xff, 0x00,
        ];
        let expected: [u8; 16] = [
            0x9a, 0x1f, 0xe1, 0xf0, 0xe8, 0xb0, 0xf4, 0x9b, 0x5b, 0x42, 0x16, 0xae, 0x79, 0x6d,
            0xa0, 0x62,
        ];
    ...
    fn test_c1_2() {
        ...
        // to be reversed.
        //
        // Sent LE Set Random Address
        // > 01 05 20 06 a9 36 3c de 52 d7
        //
        // Rcvd LE Meta (LE Connection Complete)
        // > 04 3e 13 01 00 40 00 01 00 26 0e d6 e8 c2 50 30 00 00 00 c0 03 01
        //
        // Rcvd Pairing Request
        // > 02 40 20 0b 00 07 00 06 00 01 04 00 2d 10 0e 0f
        //
        // Sent Pairing Response
        // > 02 40 00 0b 00 07 00 06 00 02 03 00 01 10 00 01
        //
        // Rcvd Pairing Confirm
        // > 02 40 20 15 00 11 00 06 00 03 4f 40 76 6b 82 fc a8 a6 05 14 99 54
        // > f5 a4 4b 3a
        //
        // Sent Pairing Confirm
        // > 02 40 00 15 00 11 00 06 00 03 dc 79 69 34 d6 98 ba d9 fd 91 79 1f
        // > 58 25 08 4f
        //
        // Rcvd Pairing Random
        // > 02 40 20 15 00 11 00 06 00 04 65 05 8d 11 0b 62 4b 30 dd 7f 7d 73
        ...
        let mut expected_rcvd_confirm_value = [
            0x4f, 0x40, 0x76, 0x6b, 0x82, 0xfc, 0xa8, 0xa6, 0x5, 0x14, 0x99, 0x54, 0xf5, 0xa4,
            0x4b, 0x3a,
        ];
        let mut expected_sent_confirm_value = [
            0xdc, 0x79, 0x69, 0x34, 0xd6, 0x98, 0xba, 0xd9, 0xfd, 0x91, 0x79, 0x1f, 0x58, 0x25,
            0x8, 0x4f,
        ];
        ...
        let k = [0x00u8; 16];
        let mut client_random = [
            0x65u8, 0x5, 0x8d, 0x11, 0xb, 0x62, 0x4b, 0x30, 0xdd, 0x7f, 0x7d, 0x73, 0x20, 0xf2,
            0xec, 0x74,
        ];
        let mut server_random = [
            0x6du8, 0xde, 0x61, 0xf5, 0x68, 0x16, 0x96, 0x67, 0x8a, 0x5e, 0x28, 0x70, 0x1a, 0x34,
            0x38, 0x0,
        ];
        ...
        let result = c1_rev(&k, &client_random, &pres, &preq, iat, &ia, rat, &ra);
        assert_eq!(result, expected_rcvd_confirm_value);

        // Validate confirm value from sender
        let result = c1_rev(&k, &server_random, &pres, &preq, iat, &ia, rat, &ra);
